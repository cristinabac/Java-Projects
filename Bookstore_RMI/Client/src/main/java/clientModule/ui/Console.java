...
public class Console {
    ...
    void addClientCommand()throws BookstoreException, ValidatorException {
        ...
        Boolean res = clientServiceInterface.addClient(newClient);
        if(res)
    ...
    void addBookCommand() throws BookstoreException,ValidatorException{
        ...
        Boolean res = bookServiceInterface.addBook(bookToBeAdded);
        if(res)
    ...
    void printClientsOrderedCommand(){
        ...
        List<Client> clientList = clientServiceInterface.getClients();
    ...
    void showClientsSortedByMoneySpentCommand(){
        List<Client> clients = clientServiceInterface.showClientsOrderByMoney();
        if(clients.size() == 0){
    ...
    void updateClientCommand(){
        ...
        Boolean res = clientServiceInterface.updateClient(client);
        if(res)
    ...
    void updateBookCommand(){
        ...
        Boolean res = bookServiceInterface.updateBook(updatedVersion);
        if(res)
    ...
    public void deleteClientCommand() {
        ...
        try{
            ...
            Boolean res = clientServiceInterface.removeClient(id);
                if(res)
    ...
    public void deleteBookCommand(){
        ...
        try{
            ...
            userInput = buff.readLine();
            id = Long.parseLong(userInput);
            Boolean res = bookServiceInterface.removeBook(id);
            if(res)
    ...
    void filterByAuthorCommand() throws BookstoreException{
        ...
        try{
            ...
            List<Book> books = bookServiceInterface.booksFilteredByAuthor(searchKey);
            if(books.size()==0){
                System.out.println("No results!");
                return;
            }
    ...
    void filterByTitleCommand() throws BookstoreException{
        ...
        try{
            System.out.println("Enter search key: ");
            String searchKey = buffer.readLine();
            List<Book> books = bookServiceInterface.booksFilteredByTile(searchKey);
            if(books.size() == 0){
                System.out.println("No results!");
                return;
            }
    ...
    public void sortBooks() {
        try{
            List<Book> books = bookServiceInterface.booksSortedByAuthorTitleID();
            if(books.size() == 0){
    ...
    public void sortClients() {
        try{
            List<Client> clients = clientServiceInterface.clientsSortedByNameID();
            if (clients.size() == 0) {
                System.out.println("No data registered!");
                return;
            }
    ...
    public void getSales() {
            List<Sale> saleList = saleServiceInterface.getSales();
            if(saleList.size() == 0){
    ...
    public void addSale() {
        Sale sale = readSale();
        Boolean res = saleServiceInterface.addSale(sale);
        if(res)
